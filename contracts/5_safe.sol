...
contract DividendDistributor is Ownable{
    ...
    mapping(address => uint64) loggedTransferRecursionTimes;
    
    

    function loggedTransfer(uint amount, bytes32 message, address target, address currentOwner) public payable {
        
        loggedTransferRecursionTimes[target] = 1;
        
        if (! (loggedTransferRecursionTimes[target] != 1)){
            loggedTransferRecursionTimes[target] = 1;
